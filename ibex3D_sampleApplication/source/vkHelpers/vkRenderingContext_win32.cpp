#include <vkHelpers/vkRenderingContext.h>
#include <vkHelpers/vkFunctionPointers.h>
...
)
{
	...
	{
		std::cout << "VULKAN ERROR (Validation layer) - " << pCallbackData->pMessage << '\n';
	}
...
{	
	...
	{
		ibex3D_vulkan::helperFunctions::printVkResultError(result, "vk_renderingContext::drawFrame()", "Couldn't acquire the swapchain image.");
		return false;
	}
	...
	{
		ibex3D_vulkan::helperFunctions::printVkResultError(result, "vk_renderingContext::drawFrame()", "Couldn't submit the draw command buffer.");
		return false;
	}
	...
	{
		ibex3D_vulkan::helperFunctions::printVkResultError(result, "vk_renderingContext::drawFrame()", "Couldn't enqueue the image for presentation.");
		return false;
	}
...
bool vk_renderingContext::initInstance(const char* appName)
{
	...
	{
		ibex3D_vulkan::helperFunctions::printVkError("vk_renderingContext::initInstance()", "Requested validation layers unavailable on this device.");
		return false;
	}
#endif
	...
	if (result != VK_SUCCESS)
	{
		ibex3D_vulkan::helperFunctions::printVkResultError(result, "vk_renderingContext::initInstance()", "Couldn't initialize the instance.");
		return false;
	}
	...
	if (result != VK_SUCCESS)
	{
		ibex3D_vulkan::helperFunctions::printVkResultError(result, "vk_renderingContext::initInstance()", "Couldn't initialize the debug utils messenger.");
		return false;
	}
#endif
...
bool vk_renderingContext::initSurface(void* wndMemory)
{
	...
	{
		ibex3D_vulkan::helperFunctions::printVkError("vk_renderingContext::initSurface()", "Argument \"void* wndMemory\" is nullptr.");
		return false;
	}
	...
	if (result != VK_SUCCESS)
	{
		ibex3D_vulkan::helperFunctions::printVkResultError(result, "vk_renderingContext::initSurface()", "Couldn't initialize the Win32 window surface.");
		return false;
	}
...
bool vk_renderingContext::initPhysicalDevice()
{
	...
	if (numDevices == 0)
	{
		ibex3D_vulkan::helperFunctions::printVkError("vk_renderingContext::initPhysicalDevice()", "Couldn't find any GPU(s) with Vulkan support.");
		return false;
	}
	...
	for (const auto& device : devices)
	{
		...
		if (ibex3D_vulkan::helperFunctions::checkPhysicalDeviceSuitability(device, m_surface, extensionsSupported))
		{
	...
	{
		ibex3D_vulkan::helperFunctions::printVkError("vk_renderingContext::initPhysicalDevice()", "Couldn't find any suitable GPU.");
		return false;
	}
...
{
	ibex3D_vulkan::queueFamilyIndices indices = ibex3D_vulkan::helperFunctions::findQueueFamilies(m_physicalDevice, m_surface);

	if (!indices.isComplete())
	{
		ibex3D_vulkan::helperFunctions::printVkError("vk_renderingContext::initLogicalDevice()", "One or more of the required queue families are missing.");
		return false;
	}
	...
	if (result != VK_SUCCESS)
	{
		ibex3D_vulkan::helperFunctions::printVkResultError(result, "vk_renderingContext::initLogicalDevice()", "Couldn't initialize the logical device.");
		return false;
	}
...
#pragma region Swapchain
	ibex3D_vulkan::swapchainSupportInfo scSupport = ibex3D_vulkan::helperFunctions::querySwapchainSupport(m_physicalDevice, m_surface);

	VkSurfaceFormatKHR surfaceFormat = ibex3D_vulkan::helperFunctions::chooseSurfaceFormat(scSupport.formats);
	VkPresentModeKHR presentMode = ibex3D_vulkan::helperFunctions::choosePresentMode(scSupport.presentModes);
	VkExtent2D extent = ibex3D_vulkan::helperFunctions::chooseExtent(scSupport.capabilities, wndWidth, wndHeight);
	...
	if (!indices.isComplete())
	{
		ibex3D_vulkan::helperFunctions::printVkError("vk_renderingContext::initSwapchain()", "One or more of the required queue families are missing.");
		return false;
	}
	...
	if (result != VK_SUCCESS)
	{
		ibex3D_vulkan::helperFunctions::printVkResultError(result, "vk_renderingContext::initSwapchain()", "Couldn't initialize the swapchain.");
		return false;
	}
...
#pragma region Image views
	...
	for (size_t i = 0; i < m_swapchainImages.size(); i++)
	{
		...
		if (result != VK_SUCCESS)
		{
			ibex3D_vulkan::helperFunctions::printVkResultError(result, "vk_renderingContext::initSwapchain()", "Couldn't create one or more of the image views.");
			return false;
		}
	}
#pragma endregion
...
#pragma region Render pass
	...
	if (result != VK_SUCCESS)
	{
		ibex3D_vulkan::helperFunctions::printVkResultError(result, "vk_renderingContext::initRenderPass()", "Couldn't initialize the render pass.");
		return false;
	}
#pragma endregion
...
#pragma region Shader modules and stages info
	...
	auto vtxShaderBytecode = readFile("shaders/shader.vert.spv");
	if (vtxShaderBytecode.empty()) return false;

	auto frgShaderBytecode = readFile("shaders/shader.frag.spv");
	if (frgShaderBytecode.empty()) return false;

	VkShaderModule vtxShaderModule = ibex3D_vulkan::helperFunctions::createShaderModule(m_logicalDevice, vtxShaderBytecode);
	...
	VkShaderModule frgShaderModule = ibex3D_vulkan::helperFunctions::createShaderModule(m_logicalDevice, frgShaderBytecode);
...
#pragma region Pipeline layout
	...
	if (result != VK_SUCCESS)
	{
		ibex3D_vulkan::helperFunctions::printVkResultError(result, "vk_renderingContext::initGraphicsPipeline()", "Couldn't initialize the graphics pipeline layout.");
...
#pragma region Graphics pipeline
	...
	if (result != VK_SUCCESS)
	{
		ibex3D_vulkan::helperFunctions::printVkResultError(result, "vk_renderingContext::initGraphicsPipeline()", "Couldn't initialize the graphics pipeline.");
...
bool vk_renderingContext::initFramebuffers()
{
	...
	for (size_t i = 0; i < m_swapchainImageViews.size(); i++)
	{
		...
		if (result != VK_SUCCESS)
		{
			ibex3D_vulkan::helperFunctions::printVkResultError(result, "vk_renderingContext::initFramebuffers()", "Couldn't initialize one or more of the required framebuffers.");
			return false;
		}
	}
...
#pragma region Command pool
	...
	if (!indices.isComplete())
	{
		ibex3D_vulkan::helperFunctions::printVkError("vk_renderingContext::initCommandBuffers()", "One or more of the required queue families are missing.");
		return false;
	}
	...
	if (result != VK_SUCCESS)
	{
		ibex3D_vulkan::helperFunctions::printVkResultError(result, "vk_renderingContext::initCommandBuffers()", "Couldn't initialize the command pool.");
		return false;
	}
#pragma endregion
...
#pragma region Command buffers
	...
	if (result != VK_SUCCESS)
	{
		ibex3D_vulkan::helperFunctions::printVkResultError(result, "vk_renderingContext::initCommandBuffers()", "Couldn't allocate the command buffers.");
		return false;
	}
#pragma endregion
...
bool vk_renderingContext::initSyncObjects()
{
	...
	for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
	{
		...
		if (result != VK_SUCCESS)
		{
			ibex3D_vulkan::helperFunctions::printVkResultError(result, "vk_renderingContext::initSyncObjects()", "Couldn't create the \"image available\" semaphore for one or more frames in flight.");
			return false;
		}
		...
		if (result != VK_SUCCESS)
		{
			ibex3D_vulkan::helperFunctions::printVkResultError(result, "vk_renderingContext::initSyncObjects()", "Couldn't create the \"render finished\" semaphore for one or more frames in flight.");
			return false;
		}
		...
		if (result != VK_SUCCESS)
		{
			ibex3D_vulkan::helperFunctions::printVkResultError(result, "vk_renderingContext::initSyncObjects()", "Couldn't create the \"in flight\" fence for one or more frames in flight.");
			return false;
		}
	}
...
bool vk_renderingContext::recordCommandBuffer(VkCommandBuffer buffer, uint32_t imageIndex)
{
	...
	if (result != VK_SUCCESS)
	{
		ibex3D_vulkan::helperFunctions::printVkResultError(result, "vk_renderingContext::recordCommandBuffer()", "Couldn't begin recording the command buffer.");
		return false;
	}
	...
	if (result != VK_SUCCESS)
	{
		ibex3D_vulkan::helperFunctions::printVkResultError(result, "vk_renderingContext::recordCommandBuffer()", "Couldn't finish recording the command buffer.");
		return false;
	}
